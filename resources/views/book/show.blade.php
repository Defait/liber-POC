...
@section('content')
        ...
        @foreach($book->chapters->sortBy('chapter_number') as $chapter)
            <ul class="collection">
                <li class="collection-item avatar">
                    <img src="{{ asset($book->information->cover_img_location) }}" alt="" class="circle responsive-img">
    ...
    @else
        ...
        @foreach($book->chapters->sortBy('chapter_number') as $chapter)
            <ul class="collection">
                <li class="collection-item avatar">
                    <img src="{{ asset($book->series->information->cover_img_location) }}" alt="" class="circle responsive-img">
                <a href="/series/{{$chapter->getSeriesObjectThroughBook($chapter->book_id)->slug}}/{{$chapter->getBookObject($chapter->book_id)->slug}}/{{$chapter->chapter_number}}"><span class="title"><strong>Chapter: {{$chapter->chapter_number}} - {{$chapter->title}}</strong></span></a>
                <p>{{str_limit($chapter->body, 500) }}</p>
                <a href="#!" class="secondary-content"><i class="material-icons">grade</i></a>
                </li>
